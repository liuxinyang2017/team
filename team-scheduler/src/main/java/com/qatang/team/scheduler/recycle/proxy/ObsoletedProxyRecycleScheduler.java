 ...
 */
@Component
@ConditionalOnProperty("scheduler.obsoleted.proxy.recycle.on")
public class ObsoletedProxyRecycleScheduler {
    private final Logger logger = LoggerFactory.getLogger(this.getClass());

    @Autowired
    private ProxyDataApiService proxyDataApiService;

    @Scheduled(fixedDelay = 60 * 60 * 1000L, initialDelay = 10 * 1000L)
    public void run() {
        try {
            logger.info(String.format("作废代理回收定时：开始处理所有状态为(%s)的代理数据", ProxyValidateStatus.FAILED.getName()));
            // 查询所有状态为 失败 且 失败次数小于等于3 的代理数据对象
            ApiRequest apiRequest = ApiRequest.newInstance()
                    .filterEqual(QProxyData.proxyValidateStatus, ProxyValidateStatus.FAILED)
                    .filterGreaterEqual(QProxyData.failedCount, GlobalConstants.PROXY_FAILED_TEST_COUNT);
            ...
            if (proxyDataList != null && !proxyDataList.isEmpty()) {
                logger.info(String.format("作废代理回收定时：查询到状态为(%s)的代理数据(%s)条", ProxyValidateStatus.FAILED.getName(), proxyDataList.size()));
                proxyDataList.forEach(proxyData -> {
                    // 循环 更新状态为 已作废
                    proxyDataApiService.updateObsoletedStatus(proxyData.getId());
                });
                logger.info(String.format("作废代理回收定时：完成(%s)条代理数据状态更新为(%s)", proxyDataList.size(), ProxyValidateStatus.OBSOLETED.getName()));
            } else {
                logger.info(String.format("作废代理回收定时：未查询到状态为(%s)的代理数据", ProxyValidateStatus.FAILED.getName()));
            }
            logger.info(String.format("作废代理回收定时：结束处理所有状态为(%s)的代理数据", ProxyValidateStatus.FAILED.getName()));
        } catch (Exception e) {

 ...
 */
@Component
public class PhaseRecycleScheduler {
    ...
    public void run() {
        for (LotteryType lotteryType : GlobalConstants.NUMBER_LOTTERY_LIST) {
            ...
            try {
                //处理彩期
                {// 将之前所有未开启的彩期全部回收
                    logger.info(String.format("彩期回收定时：开始执行(%s)将之前所有未开启的彩期全部回收", lotteryType.getName()));
                    ...
                    if (phaseList != null && !phaseList.isEmpty()) {
                        ...
                        logger.info(String.format("彩期回收定时：查询到(%s)(%s)彩期数据(%s)条，执行将之前所有未开启的彩期全部回收全部成功。", lotteryType.getName(), PhaseStatus.PENDING.getName(), phaseList.size()));
                    } else {
                        logger.info(String.format("彩期回收定时：未查询到(%s)(%s)彩期数据，无需执行将之前所有未开启的彩期全部回收", lotteryType.getName(), PhaseStatus.PENDING.getName()));
                    }
                    logger.info(String.format("彩期回收定时：结束执行(%s)将之前所有未开启的彩期全部回收", lotteryType.getName()));
                }
            } catch (Exception e) {
                logger.error(e.getMessage(), e);
            }
            logger.info(String.format("彩期回收定时：结束执行(%s)的彩期回收定时", lotteryType.getName()));
        }
    }
}

...
import com.qatang.team.scheduler.executor.data.result.IPhaseResultExecutor;
...
public class SsqResultScheduler {
    ...
    @Autowired
    @Qualifier("commonPhaseResultExecutor")
    private IPhaseResultExecutor commonPhaseResultExecutor;
    ...
    public void run() {
        try {
            ...
            if (numberLotteryDataList != null && !numberLotteryDataList.isEmpty()) {
                numberLotteryDataList.forEach(numberLotteryData -> commonPhaseResultExecutor.executeFetcher(numberLotteryData));
            } else {

 ...
 */
@Component
@ConditionalOnProperty("scheduler.waiting.test.proxy.validator.on")
public class WaitingTestProxyValidatorScheduler {
    private final Logger logger = LoggerFactory.getLogger(this.getClass());

    @Autowired
    @Qualifier("waitingTestProxyValidatorExecutor")
    private IProxyValidatorExecutor waitingTestProxyValidatorExecutor;
...
//    private ExecutorService proxyTestExecutor;
    ...
    public void run() {
        try {
            logger.info(String.format("代理测试定时：开始处理所有状态为(%s)的代理数据", ProxyValidateStatus.WAITING_TEST.getName()));
            // 查询所有状态为 待处理 的代理数据对象
...
//                CountDownLatch latch = new CountDownLatch(proxyDataList.size());
                proxyDataList.forEach(proxyData -> this.doExecute(proxyData));
//                latch.await();
    ...
    private void doExecute(ProxyData proxyData) {
//        proxyTestExecutor.submit(() -> {
            try {
                proxyDataApiService.updateBeginTestTime(proxyData.getId(), LocalDateTime.now());
                // 开始测试
                waitingTestProxyValidatorExecutor.executeValidator(proxyData);
